// ===== 知识库文件管理 hook =====
// 专管“跟后端 /api/files* 打交道”的逻辑：清单加载、上传（含三态提示）、删除、下载。
// 原则：后端是真相之源，清单从 Chroma 元数据聚合，前端不做本地记录。
...
export function useKnowledgeFiles() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        // 后端错误信息：409 重复上传、400 类型/大小/解析问题等；响应体异常时兜底默认文案，绝不静默
        ...
        // 成功也要有声音：后端用 overwritten 区分“全新上传”和“同名覆盖”
        let okMsg = "上传成功";
        try {
      ...
      setUploading(false);
      e.target.value = ""; // 清空 input，允许再次选同一文件（成败都要清，否则选同一文件无反应）
    }
  };
  ...
  const deleteFile = async (filename: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        alert(err.detail); // 403 禁删名单文案从这里直达用户
        return;
      }
  ...
  // 下载原文件：不能用 <a> 直跳（后端 404 会把整个 SPA 导航走），改用 fetch + blob：
  ...
  const downloadFile = async (filename: string) => {
    try {
      ...
      const url = URL.createObjectURL(blob); // 文件流转临时地址，下载后立刻释放，不留内存尾巴
