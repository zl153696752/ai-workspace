...
# 兼容旧引用：休眠的 LangChain 版（lc_prompt L163）和手写版（main.py L223）仍用这个常量，默认亮哥版；步骤6 删对照代码时一并清理。
PERSONA = build_persona(True)
...
# 兼容旧引用：预建的 lg_graph（L215）仍用这个亮哥版常量；4d 后 main.py 不再用 lg_graph，步骤6 清理时可一并删掉。
GRAPH_SYSTEM = build_graph_system(True)
