...
# 🔴 -2.0 是实测标定值（非拍脑袋）：bge-reranker 给中文制度文本的「相关」正文也常打负 logit
#    （eval_050 年假正文页排第 1 却只有 -0.211），门槛设 0.0 会把答案连同噪音一起误杀 → kept 空 → 答"没查到"。
#    诊断实测：正确正文页落在 -0.2 ~ -1.8，不相关垃圾片（别章节/别家文件）都在 -3.6 以下，
#    故取 -2.0：放行正文、又挡住垃圾。换 reranker 模型或语料大改后，需用评估集重新标定。
_CRAG_SCORE_THRESHOLD = -2.0
